...
static void printList(const char* key, const T* head, int pad) {
   if (!head) { indent(pad); rintf("%s = null\n", key); return; }
   indent(pad); rintf("%s = {\n", key);
...
static void printNode(const StmtNode* n, int pad) {
   ...
   if (auto x = dynamic_cast<const IfStmtNode*>(n)) {
      ...
      indent(pad + 2); rintf("integer = %s\n",   yesno(x->integer));
      indent(pad + 2); rintf("propagate = %s\n", yesno(x->propagate));
      close(n, pad);
      return;
   }
   ...
   if (auto x = dynamic_cast<const FunctionDeclStmtNode*>(n)) {
      ...
      indent(pad + 2); rintf("fnName = \"%s\"\n",    show(x->fnName));
      indent(pad + 2); rintf("nameSpace = \"%s\"\n", show(x->nameSpace));
      indent(pad + 2); rintf("package = \"%s\"\n",   show(x->package));
      indent(pad + 2); rintf("argc = %i\n", x->argc);
   ...
   if (auto x = dynamic_cast<const IntBinaryExprNode*>(n)) {
      ...
      indent(pad + 2); rintf("op = %d\n", x->op);
      indent(pad + 2); rintf("subType = %s\n", typeReqName(x->subType));
      indent(pad + 2); rintf("operand = %u\n", x->operand);
      printChild("left",  x->left,  pad + 2);
      printChild("right", x->right, pad + 2);
      close(n, pad);
      return;
   ...
   if (auto x = dynamic_cast<const StrcatExprNode*>(n)) {
      ...
      indent(pad + 2); rintf("appendChar = %d\n", x->appendChar);
      printChild("left",  x->left,  pad + 2);
      printChild("right", x->right, pad + 2);
      close(n, pad);
      return;
   ...
   if (auto x = dynamic_cast<const FuncCallExprNode*>(n)) {
      ...
      indent(pad + 2); rintf("funcName = \"%s\"\n", show(x->funcName));
      indent(pad + 2); rintf("nameSpace = \"%s\"\n", show(x->nameSpace));
      indent(pad + 2); rintf("callType = %i\n", x->callType);
      printList("args", x->args, pad + 2);
      close(n, pad);
      return;
   }
   ...
   if (auto x = dynamic_cast<const ObjectDeclNode*>(n)) {
      ...
      indent(pad + 2); rintf("parentObject = \"%s\"\n", show(x->parentObject));
      indent(pad + 2); rintf("isDatablock = %s\n", yesno(x->isDatablock));
      indent(pad + 2); rintf("isClassNameInternal = %s\n", yesno(x->isClassNameInternal));
      indent(pad + 2); rintf("isSingleton = %s\n", yesno(x->isSingleton));
